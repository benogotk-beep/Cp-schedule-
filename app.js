...
// 💅 國際化中英精簡資料庫 (已剔除面部護理、修正QQ Gel翻譯、隱藏時間標籤)
const servicesMetadata = {
    nail_hand_base: [
        ...
        { id: "h_mani", zh: "純修手 Manicure",   en: "Manicure", time: 10 },
    ...
    nail_foot_base: [
        { id: "f_pedi", zh: "純修腳 Pedicure",   en: "Pedicure", time: 15 },
...
function renderSubDetails() {
    ...
    if (bookingState.categories.nail && bookingState.parts.hand) {
        html += `<div class="sub-panel"><div class="sub-panel-title">🖐️ 手部美甲項目 / Hands Nail</div>`;
    ...
    if (bookingState.categories.nail && bookingState.parts.foot) {
        html += `<div class="sub-panel" style="border-color:#be185d;"><div class="sub-panel-title" style="background:#fce7f3; color:#be185d;">🦶 腳部美甲項目 / Feet Nail</div>`;
    ...
    if (bookingState.categories.lash) html += `<div class="sub-panel"><div class="sub-panel-title">👁️ 美睫項目 / Eyelash</div><div class="grid-2">` + generateCheckCardsHtml(servicesMetadata.lash, false) + `</div></div>`;
    ...
    return metaArray.map(item => {
        ...
        if(isAddon) {
            ...
            return `<div class="addon-card ${isSel}" onclick="handleItemClick('${item.id}')">
                        ...
                        <div>${item.zh.replace('🎨 ','').replace('✨ ','')}<br><span style="font-size:9px; font-weight:normal; color:#6b7280;">${item.en}</span></div>
                    </div>`;
        } else {
            // 主項大卡片 (隱藏時間告知)
            return `<div class="check-card ${isSel}" onclick="handleItemClick('${item.id}')">
                        <input type="checkbox" id="chk-${item.id}" ${isChk} pointer-events="none">
                        <div class="card-name">${item.zh.replace('💅 ','').replace('👁️ ','').replace('✒️ ','')}<br><span style="font-size:10px; font-weight:normal; color:#6b7280;">${item.en}</span></div>
                    </div>`;
        }
    }).join('');
}
...
function calculateTotalDuration() {
    ...
    if (sameDayChecked) {
        // 手腳同時做：取兩者最大耗時，再加上其他美睫/紋綉大類
        bookingState.totalDuration = Math.max(handSum, footSum) + otherSum;
    } else {
        // 預設分開做：時間完整重疊疊加
        bookingState.totalDuration = handSum + footSum + otherSum;
    }
...
// 🧠 精算可用時間空檔 (基本9小時 + 10分鐘雙向緩衝 + 系統分配零沙漏規則)
function 精算可用時間空檔(dayNumber, year, month) {
    ...
    if(targetsStaffs.length === 0) {
        container.innerHTML = `<div style="grid-column: span 3; text-align:center; padding:15px; color:#ef4444; font-weight:bold; font-size:12px;">❌ 當天沒有可用人手 / No staff available</div>`;
        return;
    }
    ...
    timeCandidates.forEach(slotStart => {
        ...
        for (let staff of targetsStaffs) {
            ...
            let shiftEndMins = shiftStartMins + (9 * 60); // 9小時常態制
            ...
            // 🌟 核心新規則：如果客人選系統分配，直接踢走、隱藏所有帶有沙漏的黃燈時段
            if (selectedStaff === 'auto' && currentSlotReview) {
                continue; 
            }
...
function selectTimeSlot(time, staff, isReview) {
    ...
    document.getElementById('custom-time-select').value = ""; // 選了標準按鈕，就清空下方自選下拉
    document.getElementById('btn-submit-booking').disabled = false;
}

// 🔧 處理 09:00 - 23:00 自選特殊時段選動
...
function toggleQuickTag(tagId) {
    ...
    if(tagId === 'sameday') {
        calculateTotalDuration(); // 勾選手腳同做，立即刷新時間加總
    }
}

// 🚀 一鍵無縫封包跳轉 WhatsApp 官方發送
function executeWhatsAppBooking() {
    ...
    allMeta.forEach(item => {
        if(bookingState.selectedItems.has(item.id)) {
            zhItems.push(item.zh); enItems.push(item.en);
        }
    });
